 ...
 *
 * @param id                电机编号
 * @param reducation_ratio  减速器减速比，轮子转一圈，电机需要转的圈数
 * @param pulse_ration      脉冲比，电机转一圈所产生的脉冲数
 * @param wheel_diameter    轮子的外直径，单位mm
 */
 ...
 * @brief 设置运动学相关参数
 *
 * @param wheel_distanceA
 * @param wheel_distanceB
 */
void Kinematics::set_kinematic_param(float wheel_distanceA, float wheel_distanceB)
{
    wheel_distance_a = wheel_distanceA;
    wheel_distance_b = wheel_distanceB;
}
/**
 * @brief 更新轮子的tick数据
 *
...
void Kinematics::update_bot_odom_(uint32_t dt)
{
    ...
    /*更新x和y轴上移动的距离*/
    odom_.x += odom_.linear_x_speed / odom_.angular_speed * (sin(odom_.yaw + odom_.angular_speed * dt_s) - sin(odom_.yaw)) + odom_.linear_y_speed / odom_.angular_speed * (cos(odom_.yaw + odom_.angular_speed * dt_s) - cos(odom_.yaw));
    odom_.y += odom_.linear_y_speed / odom_.angular_speed * (sin(odom_.yaw + odom_.angular_speed * dt_s) - sin(odom_.yaw)) - odom_.linear_x_speed / odom_.angular_speed * (cos(odom_.yaw + odom_.angular_speed * dt_s) - cos(odom_.yaw));
 ...
 *
 * @param linear_x_speed
 * @param linear_y_speed
 * @param angular_speed
 * @param out_wheel_speed1
 * @param out_wheel_speed2
 * @param out_wheel_speed3
 * @param out_wheel_speed4
 */
                                   ...
                                   float &out_wheel_speed1, float &out_wheel_speed2, float &out_wheel_speed3, float &out_wheel_speed4)
{
    ...
    out_wheel_speed1 = linear_x_speed - linear_y_speed - angular_speed * (Kinematics::wheel_distance_a + Kinematics::wheel_distance_b);
    out_wheel_speed2 = linear_x_speed + linear_y_speed + angular_speed * (Kinematics::wheel_distance_a + Kinematics::wheel_distance_b);
    out_wheel_speed3 = linear_x_speed + linear_y_speed - angular_speed * (Kinematics::wheel_distance_a + Kinematics::wheel_distance_b);
    out_wheel_speed4 = linear_x_speed - linear_y_speed + angular_speed * (Kinematics::wheel_distance_a + Kinematics::wheel_distance_b);
                                   ...
                                   float &linear_x_speed, float &linear_y_speed, float &angular_speed)
{
    ...
    linear_x_speed = motor_param_->wheel_diameter * (wheel1_speed + wheel2_speed + wheel3_speed + wheel4_speed) / 4.0f;
    linear_y_speed = motor_param_->wheel_diameter * (-wheel1_speed + wheel2_speed + wheel3_speed - wheel4_speed) / 4.0f;
    angular_speed = motor_param_->wheel_diameter * float(-wheel1_speed + wheel2_speed - wheel3_speed + wheel4_speed) / (4.0f * (Kinematics::wheel_distance_a + Kinematics::wheel_distance_b));
